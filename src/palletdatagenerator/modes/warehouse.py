...
class WarehouseMode(BaseGenerator):
    ...
    def detect_faces_by_rack_geometry(self, cam_obj, sc):
        ...
        visible_faces = []
        min_area = (
            self.config.get("keypoints_min_face_area", 100) * 0.05
        )  # Very lenient
        ...
        print(
            f"📐 Camera forward direction: ({cam_forward.x:.2f}, {cam_forward.y:.2f}, {cam_forward.z:.2f})"
        )
        ...
        print(
            f"🎯 Filtered {len(all_pallets)} pallets to {len(filtered_pallets)} based on camera view direction"
        )
        ...
        for pallet_obj in filtered_pallets:
            ...
            for face_data in side_faces:
                ...
                if not skip_occlusion:
                    if not self.is_face_visible_simple(
                        cam_pos, face_center_3d, pallet_obj
                    ):
                ...
                pallet_visible_faces.append(
                    {
                        ...
                        "visibility_score": abs(dot_product)
            ...
            if pallet_visible_faces:
                pallet_visible_faces.sort(
                    key=lambda x: x["visibility_score"], reverse=True
                )
    ...
    def analyze_rack_structure(self):
        ...
        if left_collection:
            print(
                f"📦 Analyzing left collection with {len(left_collection.objects)} objects"
            )
            pallets = [
                obj
                for obj in left_collection.objects
                if obj.type == "MESH"
                and ("pallet" in obj.name.lower() or obj.pass_index > 0)
            ]
            ...
            if pallets:
                ...
                print(
                    f"   Left rack centerline: ({avg_pos.x:.2f}, {avg_pos.y:.2f}, {avg_pos.z:.2f})"
                )

        # Analyze right rack
        if right_collection:
            print(
                f"📦 Analyzing right collection with {len(right_collection.objects)} objects"
            )
            pallets = [
                obj
                for obj in right_collection.objects
            ...
            if pallets:
                ...
                print(
                    f"   Right rack centerline: ({avg_pos.x:.2f}, {avg_pos.y:.2f}, {avg_pos.z:.2f})"
                )
        ...
        if racks_info["left"] and racks_info["right"]:
            ...
            print(
                f"   Aisle center: ({racks_info['aisle_center'].x:.2f}, {racks_info['aisle_center'].y:.2f}, {racks_info['aisle_center'].z:.2f})"
            )
    ...
    def select_candidate_pallets_by_position(self, cam_pos, cam_dir, racks_info):
        ...
        if not racks_info["aisle_center"]:
            ...
            if racks_info["left"]:
                print(
                    f"   Adding {len(racks_info['left']['pallets'])} pallets from left rack"
                )
                for pallet in racks_info["left"]["pallets"]:
                    candidates.append({"object": pallet, "side": "left_aisle"})
            if racks_info["right"]:
                print(
                    f"   Adding {len(racks_info['right']['pallets'])} pallets from right rack"
                )
        ...
        print(
            f"   Aisle width: {aisle_width:.2f}m, center threshold: {center_threshold:.2f}m"
        )
